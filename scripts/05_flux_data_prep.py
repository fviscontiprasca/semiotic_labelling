"""
File-based data preparation for Flux.1d LoRA fine-tuning.
Adapted from full fine-tuning pipeline with diverse prompt templates and
enhanced semiotic conditioning for LoRA training.
"""
...
@dataclass
class LoRATrainingDataConfig:
    """Configuration for LoRA fine-tuning data preparation."""
    
    # Quality thresholds - more lenient for LoRA
    min_semiotic_score: float = 0.3
    min_image_size: Tuple[int, int] = (512, 512)
    max_image_size: Tuple[int, int] = (1536, 1536)
    target_resolution: int = 1024
    min_caption_length: int = 15
    max_caption_length: int = 200
    
    # Data augmentation for LoRA
    use_caption_augmentation: bool = True
    use_semiotic_token_augmentation: bool = True
    augmentation_probability: float = 0.4
    
    # Memory optimization - less aggressive for LoRA quality
    use_webp_compression: bool = False
    webp_quality: int = 95
    
    # Enhanced semiotic conditioning - more flexible for LoRA
    require_architectural_style: bool = False
    require_urban_mood: bool = False
    enhanced_prompt_templates: bool = True

class FileBasedLoRADataPreparator:
    """File-based data preparation for Flux.1d LoRA fine-tuning."""
    
    def __init__(self, base_path: str = ".", output_path: Optional[str] = None, config: Optional[LoRATrainingDataConfig] = None):
        """Initialize file-based LoRA data preparation system."""
        
        self.base_path = Path(base_path)
        # Align with LoRA training outputs structure
        default_output = self.base_path / "data" / "outputs" / "05_flux_lora_training_data"
        self.output_path = Path(output_path) if output_path else default_output
        
        self.config = config or LoRATrainingDataConfig()
    ...
    def prepare_training_data(self, max_samples: Optional[int] = None) -> Dict[str, Any]:
        """Prepare enhanced training data for LoRA Flux.1d fine-tuning."""
        
        logger.info("🚀 Starting file-based LoRA training data preparation")
        ...
        # Get all individual semiotic feature files (exclude aggregate files)
        all_files = list(self.semiotic_input_dir.glob("*_semiotic_features.json"))
        semiotic_files = [f for f in all_files if f.name != "all_semiotic_features.json"]
        logger.info(f"Found {len(semiotic_files)} semiotic feature files")
        
        if max_samples and len(semiotic_files) > max_samples:
            semiotic_files = semiotic_files[:max_samples]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Prepare LoRA training data with diverse captions")
    ...
    parser.add_argument("--min_semiotic_score", type=float, default=0.3, help="Minimum semiotic score for LoRA")
    ...
    metadata = preparator.prepare_training_data(max_samples=args.max_samples)
    
    print(f"\n✅ LoRA training data prepared successfully!")
    print(f"📊 Total samples: {metadata.get('summary', {}).get('total_samples', 0)}")
